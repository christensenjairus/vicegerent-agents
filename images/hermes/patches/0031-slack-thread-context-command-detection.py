#!/usr/bin/env python3
"""Vicegerent patch: stop Slack thread-context injection from masking
in-thread slash/bang commands (``/model``, ``!model``, ``/reasoning``,
``!reasoning``, ``/stop``, etc.).
...
starts with ``/`` (or the rewritten ``!``->``/`` prefix -- see patch 0006's
sibling logic for that rewrite, which happens on ``original_text``
BEFORE this concatenation and is therefore unaffected itself). The
message is silently reclassified as ordinary chat text and every slash
command silently no-ops for the rest of that thread turn: ``/model``,
``!model``, ``/reasoning``, ``!reasoning``, ``/stop``, ``/queue``, etc. all
fail to dispatch, with zero error message -- the agent just answers the
literal text conversationally instead.
...
``gateway/run.py`` already consumes it correctly and late (line ~11582,
well after every command-dispatch check has already run):
...
Fix
---
1. Stop mutating ``text`` with the thread-context prefix; keep ``text``
   as the raw (mention-stripped) trigger message throughout, so command
   detection downstream sees only the actual command the user typed.
2. Stash the fetched context in a new ``_slack_thread_channel_context``
   local instead.
3. Pass it through ``MessageEvent(..., channel_context=_slack_thread_channel_context, ...)``
   so ``gateway/run.py`` still gets the full thread background -- just via
   the field designed for it, applied after command dispatch instead of
   before.

This is purely a routing fix: the thread-context content itself, and
when it's fetched, are unchanged. A command sent in a fresh thread (no
thread-context fetch triggered) was never affected and continues to work
identically; a plain conversational message in an existing thread still
gets exactly the same background context prepended to what the LLM sees,
just via ``channel_context`` instead of a mutated ``text``.

Fail-loud by design: if either anchor is absent or appears an unexpected
number of times (upstream refactored this function), the patch raises
...
APPLIED_MARKER = "Vicegerent patch 0031"
...
def main() -> int:
    ...
    src += (
        f"\n# {APPLIED_MARKER}: Slack thread-context now routed through "
