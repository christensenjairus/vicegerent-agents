...
way is the whole design — duplicated allowlists drift out of sync and that drift
is a security bug.

| Layer | Job | What it is NOT |
| --- | --- | --- |
| **agentgateway** tool-name allowlist | The single gate for **which tools an agent may call at all**. | — |
...
If you ever find yourself adding a tool or kind name to the shim mapping or the
Cerbos `allow` rule just to *permit* it, stop — that belongs in the gateway
allowlist. The shim/Cerbos only ever grow to block a *new* sensitive resource.

### Known limitation: the guardrail must be attached for any of this to apply

The entire Secret block is conditional on the `AgentgatewayPolicy` actually
attaching the `tools/call -> mcp-cerbos-shim` guardrail (with `failureMode:
FailClosed`) to the Kubernetes backend. `FailClosed` only governs what happens
once the processor is *invoked and fails* (shim down, Cerbos unreachable) — it
does **not** cover the processor never being attached. If the guardrail block is
removed/renamed/weakened, the gateway forwards straight to the MCP server with no
shim or Cerbos check: a silent fail-open.
...
2. Parses the JSON-RPC params (`{name, arguments}`); unparseable/missing → deny.
...
4. Evaluates the mapped tool's CEL expressions against `{tool, args, backend,
   ...
   `canonicalK8s` helper). A CEL eval failure → deny (the shim's own
   malfunction; never send a half-built resource).
5. Calls Cerbos `IsAllowed`. Allowed → `Pass{}`; denied or Cerbos error → `AuthorizationError`.
...
kind can't be resolved → `kind==""` → `deny-no-kind`); the *permit* decision is
